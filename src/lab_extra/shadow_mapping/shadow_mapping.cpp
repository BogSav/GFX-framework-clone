...
void ShadowMapping::Init()
{
...

	{
		...
		lightCamera->SetPerspective(90, window->props.aspectRatio, 0.01f, 200);
		lightCamera->m_transform->SetMoveSpeed(2);
		lightCamera->m_transform->SetWorldPosition(glm::vec3(0, 4.5f, 2.5));
		lightCamera->m_transform->SetWorldRotation(glm::vec3(-90, 0, 0));
		lightCamera->Update();
