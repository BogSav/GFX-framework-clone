...
{
public:
	BoundingBox(bool wireframe = false) : m_wireframe(wireframe)
	{
	...
	void SetBottomLeftCorner(glm::vec2 pos) { m_bottomLeftCorner = pos; }
	void SetUpperRightCorner(glm::vec2 pos) { m_upperRightCorner = pos; }

	glm::vec2 GetBottomLeftCorner() const { return m_bottomLeftCorner; }
	glm::vec2 GetUpperRightCorner() const { return m_upperRightCorner; }
	...
	BoundingBox GetBoundingBoxTransformed(glm::mat3 trMatrix) const
	{
		...
		glm::vec3 uprCorner =
			trMatrix * glm::vec3{GetUpperRightCorner()[0], GetUpperRightCorner()[1], 1};
		glm::vec3 bolCorner =
			trMatrix * glm::vec3{GetBottomLeftCorner()[0], GetBottomLeftCorner()[1], 1};
