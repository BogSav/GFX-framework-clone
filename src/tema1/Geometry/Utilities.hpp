...
{
	return glm::transpose(glm::mat3(direction, 0, 0, 0, 1, 0, 0, 0, 1));
}

inline glm::mat3 ReflectionMatrixOX(int direction)
{
	return glm::transpose(glm::mat3(1, 0, 0, 0, direction, 0, 0, 0, 1));
}
...
inline glm::mat3 VisualizationTransf2DUnif(
	const LogicSpace& logicSpace, const ViewportSpace& viewSpace)
{
	...
	if (sx < sy)
		...
		smin = sy;
	tx = viewSpace.GetX() - smin * logicSpace.GetX()
		+ (viewSpace.GetWidth() - smin * logicSpace.GetWidth()) / 2;
	ty = viewSpace.GetX() - smin * logicSpace.GetY()
		+ (viewSpace.GetHeight() - smin * logicSpace.GetHeight()) / 2;
