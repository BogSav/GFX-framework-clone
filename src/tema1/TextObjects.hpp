...
private:
    ...
    {
        m_textRenderer = new gfxc::TextRenderer(m_window->props.selfDir, m_resolution.x, m_resolution.y);
        m_textRenderer->Load(PATH_JOIN(m_window->props.selfDir, RESOURCE_PATH::FONTS, "Hack-Bold.ttf"), 18);
    }
protected:
...
public:
	void RenderMenu()
	{
        m_textRenderer->RenderText("START", 
...
public:
    void RenderLevelStatus(int currentLevel, int nrOfLevels)
    {
        m_textRenderer->RenderText(
            "Level: " + std::to_string(currentLevel) + " / " + std::to_string(nrOfLevels) + " - " + m_difficulties[currentLevel],
            8.f,
            8.f,
            m_scale,
            m_textColor);
...
public:
    ...
    {
        m_textRenderer->RenderText("GAME OVER",
            m_resolution.x / 2.f - 100.f,
            m_resolution.y / 2.f - 20.f,
            m_scale,
            m_textColor);
        if(gameWon)
            m_textRenderer->RenderText("YOU WON",
                m_resolution.x / 2.f - 80.f,
                m_resolution.y / 2.f + 60.f,
                m_scale,
                m_textColor);
        else
            m_textRenderer->RenderText("YOU LOST",
                m_resolution.x / 2.f - 80.f,
                m_resolution.y / 2.f + 60.f,
                m_scale,
                m_textColor);
