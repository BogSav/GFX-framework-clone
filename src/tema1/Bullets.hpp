	...
	public:
		Bullet(glm::vec3 v)
		{
			m_geometries.emplace_back(
				new Rectangle("body", {v[0], 0}, 0.8f, 1.f, Colors::LIGHT_GOLD, 0.7f));
			m_geometries.emplace_back(new Circle(
				"head", 10, Colors::DARK_GOLD, 0.4f, glm::vec3{0.4f + v[0], 1.f, 0}, 0.6f));
		}
...
public:
	Bullets(
		...
		: Object(logicSpace, viewPortSpace, shader, camera), m_nrOfBullets(3)
	{
		ResetBullets();
	}
	...
	int GetRemainingBullets() { return static_cast<int>(m_bullets.size()); };
