...
void main()
{
    ...
    float ambient_light = material_kd * 0.1;
    ...
    float factorAtenuare = 1 / (pow(distance(world_pos, light_position), 2) + 1);
    vec3 culoareObiect =  object_color * min((ambient_light + factorAtenuare * ( diffuse_light + specular_light )) * 2 * abs(sin(light_change[0])), 1);
